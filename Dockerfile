...
# 1. Install system dependencies (Required for Postgres and Laravel)
...
# 3. Install PHP extensions (pdo_pgsql is the star here)
...
# 5. Install Dependencies (Using --no-scripts to bypass DB errors during build)
...
# We use 'sleep 10' to ensure the network is ready for the Pooler connection.
CMD sleep 10 && \
